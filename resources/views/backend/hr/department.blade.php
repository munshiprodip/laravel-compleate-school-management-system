    ...
    <div class="col-12">
        ...
        <div class="card card-dark">
            <div class="card-header">
                ...
                <h3 class="card-title"><span><a onclick="addDepartment()" href="#" class="btn btn-outline-info">ADD </button></a></span> Department</h3>
...
@section('script')
    <script type="text/javascript">
        ...
        function addDepartment() {
            $('#addDepartment form')[0].reset();
            $("#addDepartment").modal('show');
        }

        $(function () {
            $('#addDepartment form').on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    $.ajax({
                        url: "{{route('create.department')}}",
                        type: "POST",
        ...
        $(function () {
            $('#editDepartment form').on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    $.ajax({
                        url: "{{url('department')}}"+'/'+id,
                        type: "POST",
                        data: $('#editDepartment form').serialize(),
                        success:function (data) {
                            const Toast = Swal.mixin({
                                toast: true,
                                position: 'top-end',
        ...
        function changeStatus(id){
            swal.fire({
                ...
                if (e.value === true){
                    var url = "{{url('department/status/change')}}"+'/'+id;
                    $.ajax({
                        url:url,
                        type:"GET",
                        dataType:"JSON",
